...
import { ArrowLeft, Plus, Users, Edit, Save, X, Trash2 } from "lucide-react";
...
export default function WorkoutDetailView({ workout, clients, onBack, onWorkoutUpdated, onWorkoutDeleted, onAssignWorkout }) {
  ...
  const assignedClient = clients.find(c => c.id === workout.client_id);
  ...
  const handleDeleteWorkout = async () => {
    ...
    try {
      ...
      if (relatedLogs.length > 0) {
        ...
        // Show more detailed message for assigned workouts
        if (workout.client_id) {
          toast.success(
            `Deleted client workout and ${relatedLogs.length} related log${relatedLogs.length > 1 ? 's' : ''}`,
            { duration: 3000 }
          );
        } else {
          toast.success(
            `Deleted workout template and ${relatedLogs.length} related log${relatedLogs.length > 1 ? 's' : ''}`,
            { duration: 3000 }
          );
        }
      } else {
        toast.success(workout.client_id ? "Client workout deleted successfully" : "Workout template deleted successfully");
      }
          ...
          <>
            ...
            {workout.client_id && (
              <p className="text-sm text-yellow-400 mb-2">
                ⚠️ Warning: All workout history logs for this client's program will also be deleted.
              </p>
            )}
            {workout.is_template !== false && (
              <p className="text-sm text-muted-foreground">
                Note: Workouts assigned to clients from this template will not be affected.
              </p>
            )}
          </>
        }
