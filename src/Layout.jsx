  ...
  const ensureInAvailablePool = useCallback(async (userData) => {
    try {
      ...
      if (existing.length === 0) {
        ...
        console.log(`User ${userData.email} successfully added to pool.`);
      }
    } catch (error) {
       // Silent fail for auth errors, log others
        ...
        <Sidebar className="border-r border-gray-200 bg-gray-100 backdrop-blur-xl">
                  ...
                  {navigationItems.map((item) => (
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild className={`hover:bg-gray-300/50 hover:text-gray-900 transition-all duration-300 rounded-xl mb-2 group ${location.pathname === item.url ? 'bg-gray-300 text-gray-900' : 'text-gray-700 hover:text-gray-900'}`}>
                        <Link to={item.url} className="bg-gray-50 text-gray-900 mb-2 px-4 py-3 text-sm text-left rounded-xl flex w-full items-center gap-2 h-8 hover:bg-gray-300/50 hover:text-gray-900 transition-all duration-300 group gap-3">
          ...
          <SidebarFooter className="bg-neutral-900 p-4 flex flex-col gap-2 border-t border-gray-200 space-y-4">
             <div className="flex items-center justify-between">
              ...
              <button onClick={toggleTheme} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-400'}`}>
        ...
        <main className="flex-1 flex flex-col">
          <header className={`${isDarkMode ? 'bg-gray-900/30' : 'bg-white/30'} backdrop-blur-xl border-b ${isDarkMode ? 'border-gray-800' : 'border-gray-200'} px-6 py-4`}>
            <div className="flex items-center gap-4">
              <SidebarTrigger className={`md:hidden ${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-100'} p-2 rounded-lg`} />
              <div className="flex items-center gap-2 flex-1">
                  <h1 className="text-xl font-bold text-foreground">APX PERFORMANCE</h1>
                  <span className="text-xs text-gray-500 uppercase tracking-wide">{user?.user_type === "coach" ? "Coach" : "Client"}</span>
              </div>
