...
export default function NutritionPlanner() {
  ...
  const handlePlanAssigned = async (planId, clientIds) => {
    ...
    try {
      ...
      if (failureCount === 0) {
        toast.success(`Nutrition plan assigned to ${successCount} client${successCount > 1 ? 's' : ''}!`);
      } else if (successCount > 0) {
        toast.warning(`Partially assigned: ${successCount} succeeded, ${failureCount} failed. Please try again for failed assignments.`);
      } else {
        toast.error("Failed to assign nutrition plan to any clients. Please try again.");
      }
    } catch (error) {
